...
struct HkdfLabel {
    bytes: [u8; MAX_HKDF_LABEL_LEN],
    len: usize,
}

impl AsRef<[u8]> for HkdfLabel {
    fn as_ref(&self) -> &[u8] {
        &self.bytes[..self.len]
    }
}
...
impl Hkdf {
    ...
    pub fn expand_label(
        ...
        self.expand(prk, info.as_ref(), out)
    }
    ...
    fn hkdf_label(label: &str, context: &[u8], out_len: usize) -> Result<HkdfLabel, HkdfError> {
        ...
        let len = 2 + 1 + label_with_prefix_len + 1 + context.len();
        let mut bytes = [0u8; MAX_HKDF_LABEL_LEN];
        bytes[..2].copy_from_slice(&total_len.to_be_bytes());
        bytes[2] = label_len;
        bytes[3..9].copy_from_slice(b"tls13 ");
        let label_end = 9 + label.len();
        bytes[9..label_end].copy_from_slice(label.as_bytes());
        bytes[label_end] = context_len;
        bytes[label_end + 1..len].copy_from_slice(context);
        Ok(HkdfLabel { bytes, len })
    }
}

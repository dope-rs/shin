...
impl<C: connection::Clock, K: kx::Initiator> Negotiation for client::Client<C, K> {
    fn handle_server_hello<S: connection::EventSink + ?Sized>(
        &mut self,
        sh: views::ServerHelloRef<'_>,
        raw: &[u8],
        ...
        if let Some(ext) = psk_ext {
            ...
            if self.session.handshake.resumption.is_none() {
        ...
        if self.session.kx.pending_group().map(kx::KexGroup::wire_id) != Some(server_share.group())
        {
            return Err(connection::Error::IllegalParameter.into());
        }
        let dhe = self
        ...
        let active_resumption = self.session.handshake.resumption.take();
    ...
    fn handle_hello_retry_request<S: connection::EventSink + ?Sized>(
        ...
        if self.session.kx.pending_group() == Some(group) {
            return Err(connection::Error::IllegalParameter.into());
        }
        ...
        self.finish_hello_retry_request(group, cookie, events)
    }
    ...
    fn finish_hello_retry_request<S: connection::EventSink + ?Sized>(
        ...
        let mut resumption = self.session.handshake.resumption.take();
        let now_ms = connection::Clock::now_ms(&self.session.runtime.clock);
        ...
        let certificate_types = self.session.certificate_type_offers();
    ...
    fn handle_encrypted_extensions<S: connection::EventSink + ?Sized>(
        ...
        let offered_early_data = match self.session.extensions.early_data {
        ...
        let negotiated = session::NegotiatedExtensions::decode(
            ee.extensions,
            &self.session.offer.config,
            self.session.certificate_type_offers(),
            offered_early_data.is_some() && !self.session.handshake.hrr_done,
        )?;
        let (next_early_data, early_data_event) = match (offered_early_data, negotiated.early_data)
        {
            (Some(authority), Some(_)) => {
                let selected_suite = self
                    .session
                    .application
                    .traffic
                    .suite()
                    .ok_or(connection::Error::UnexpectedMessage)?;
                if self.session.handshake.hrr_done
                    || !self.session.handshake.psk_used
        ...
        self.session.extensions.selected_alpn = negotiated.alpn;
        if let Some(next) = next_early_data {
            self.session.extensions.early_data = next;
        }

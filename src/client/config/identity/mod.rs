...
use crate::identity::{self, spki};
use crate::wire::handshake::{self, messages};
use alloc::vec;
    ...
    pub(super) fn validate(&self) -> Result<(), super::Error> {
        let valid = match self {
            ...
            Self::X509 {
                ...
                messages::Certificate::chain_fits(chain_der)
                    && signing_key.matches_x509_chain(chain_der)
                    && self.outbound_flight_capacity() <= handshake::MAX_SIZE
            }
        };
        if valid {
            Ok(())
        ...
        let certificate_len = match self {
            Self::RawPublicKey { signing_key } => {
                ...
                let Some(pubkey) = signing_key.pubkey() else {
                    return handshake::MAX_SIZE;
                };
                let spki_len = spki::SubjectPublicKey::encoded_ed25519(pubkey).len();
                FIXED_MESSAGE_BYTES + ENTRY_FRAMING_BYTES + spki_len
            }
            Self::X509 { chain_der, .. } => {
                messages::Certificate::chain_message_len(chain_der).unwrap_or(handshake::MAX_SIZE)
            }
        };
        certificate_len
            + messages::CertificateVerify::frame_len(self.signing_key().signature_len_upper_bound())
            + FINISHED_MAX_LEN
    }

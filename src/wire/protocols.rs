...
impl SignatureAlgorithms {
    ...
    pub(crate) fn contains(data: &[u8], scheme: u16) -> Result<bool, codec::DecodeError> {
        let mut reader = codec::Reader::new(data);
...
impl<'a> KeyShares<'a> {
    ...
    pub(crate) fn select_client_entry(
        self,
        prefer: &[kx::KexGroup],
    ) -> Option<(kx::KexGroup, &'a [u8])> {
        let mut r = codec::Reader::new(self.encoded);
    ...
    pub(crate) fn decode_hrr(data: &[u8]) -> Result<u16, codec::DecodeError> {
        let mut r = codec::Reader::new(data);
        let group = r.u16()?;
        r.finish()?;
        Ok(group)
    ...
    pub(crate) fn decode_server(data: &[u8]) -> Result<(u16, &[u8]), codec::DecodeError> {
        let mut r = codec::Reader::new(data);
        let group = r.u16()?;

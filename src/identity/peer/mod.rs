...
use crate::crypto::sig::{self, VerifyingKey};
...
pub(crate) struct LeafKey {
    ...
    raw: ArrayVec<u8, MAX_PEER_KEY_LEN>,
}
...
impl LeafKey {
    pub(crate) fn from_raw(kind: LeafKeyKind, raw: &[u8]) -> Result<Self, Error> {
        let mut stored = ArrayVec::new();
        stored
            .try_extend_from_slice(raw)
            .map_err(|_| Error::UnsupportedSigScheme)?;
    ...
    pub(crate) fn parse_x509(leaf_der: &[u8]) -> Result<(Self, &[u8]), Error> {

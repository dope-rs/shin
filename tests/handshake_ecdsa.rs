...
fn ecdsa_p256_self_signed() -> (Vec<u8>, SigningKey) {
    let key = KeyPair::generate_for(&PKCS_ECDSA_P256_SHA256).unwrap();
    let pkcs8 = key.serialize_der();
    let signing = SigningKey::from_ecdsa_p256_pkcs8(&pkcs8).unwrap();
...
#[test]
fn handshake_with_ecdsa_p256_x509_chain() {
    let (cert_der, signing) = ecdsa_p256_self_signed();
    ...
    assert!(has_done(&c3), "client confirmed via ECDSA-P256 chain");

...
fn chacha_record_does_not_open_under_aes() {
    let mut chacha = Sealer::with_suite(&SECRET, CipherSuite::ChaCha20Poly1305Sha256).unwrap();
    let mut aes = Sealer::with_suite(&SECRET, CipherSuite::Aes128GcmSha256).unwrap();

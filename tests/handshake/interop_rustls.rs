...
use shin::wire::record::{CipherSuite, ContentType, Opener, Plaintext, Sealer};
...
fn peer_extension(events: &[Event], ty: u16) -> Option<&[u8]> {
    events.iter().find_map(|event| match event {
        Event::PeerExtension { ty: event_ty, data } if *event_ty == ty => Some(data.as_slice()),
        _ => None,
    })
}
...
fn rustls_quic_server(
    suite: CipherSuite,
    cert: &TestCert,
    transport_params: &[u8],
) -> rustls::quic::ServerConnection {
    let provider = ring_provider(suite);
    let key = PrivateKeyDer::Pkcs8(PrivatePkcs8KeyDer::from(cert.pkcs8_der.clone()));
    let mut config = rustls::ServerConfig::builder_with_provider(provider)
        .with_protocol_versions(&[&rustls::version::TLS13])
...
fn rustls_quic_client(
    suite: CipherSuite,
    cert: &TestCert,
    transport_params: &[u8],
) -> rustls::quic::ClientConnection {
    let provider = ring_provider(suite);
    let verifier = PinnedServerVerifier {
        cert: cert.cert_der.clone(),
        provider: provider.clone(),
...
fn shin_quic_server(cert: &TestCert, transport_params: &[u8]) -> Server<impl Fn() -> u64> {
    let now_ms = cert.now_ms;
    let seed = extract_ed25519_seed(&cert.pkcs8_der).expect("seed");
    Server::new_with_transport(
        ServerConfig {
            source: CertSource::X509 {
                chain_der: vec![cert.cert_der.clone()],
                signing_key: SigningKey::from_seed(&seed).unwrap(),
...
fn drive_shin_client_vs_rustls_server(suite: CipherSuite) {
    ...
    feed_rustls(
        &mut server,
        &Plaintext::encode(ContentType::Handshake, &ch).unwrap(),
    );
...
fn drive_shin_server_vs_rustls_client(suite: CipherSuite) {
    ...
    let mut to_client = Plaintext::encode(ContentType::Handshake, &sh).unwrap();
...
fn drive_shin_quic_client_vs_rustls_server(suite: CipherSuite) {
    const CLIENT_PARAMS: &[u8] = b"shin quic client params";
    const SERVER_PARAMS: &[u8] = b"rustls quic server params";
    let cert = gen_ed25519_cert();
    let mut server = rustls_quic_server(suite, &cert, SERVER_PARAMS);
    let mut client = shin_quic_client(suite, &cert, CLIENT_PARAMS);

    let client_hello = find_send(&client.start().unwrap(), Epoch::Plaintext);
    server.read_hs(&client_hello).unwrap();

    let mut server_hello = Vec::new();
    assert!(matches!(
        server.write_hs(&mut server_hello),
        Some(rustls::quic::KeyChange::Handshake { .. })
    ));
    let handshake_keys = client.read(Epoch::Plaintext, &server_hello).unwrap();
    assert!(handshake_keys.iter().any(|event| matches!(
        event,
        Event::KeysReady {
    ...
    assert!(matches!(
        server.write_hs(&mut server_finished),
        Some(rustls::quic::KeyChange::OneRtt { .. })
    ));
    let client_finished = client.read(Epoch::Handshake, &server_finished).unwrap();
    assert!(has_done(&client_finished));
    assert_eq!(
        peer_extension(
            &client_finished,
            extension::Type::QUIC_TRANSPORT_PARAMETERS.0,
        ),
        Some(SERVER_PARAMS),
    );
    server
        .read_hs(&find_send(&client_finished, Epoch::Handshake))
        .unwrap();
...
fn drive_shin_quic_server_vs_rustls_client(suite: CipherSuite) {
    const CLIENT_PARAMS: &[u8] = b"rustls quic client params";
    const SERVER_PARAMS: &[u8] = b"shin quic server params";
    let cert = gen_ed25519_cert();
    let mut client = rustls_quic_client(suite, &cert, CLIENT_PARAMS);
    let mut server = shin_quic_server(&cert, SERVER_PARAMS);

    let mut client_hello = Vec::new();
    assert!(client.write_hs(&mut client_hello).is_none());
    let server_flight = server.read(Epoch::Plaintext, &client_hello).unwrap();
    assert_eq!(
        peer_extension(&server_flight, extension::Type::QUIC_TRANSPORT_PARAMETERS.0,),
        Some(CLIENT_PARAMS),
    );
    let server_hello = find_send(&server_flight, Epoch::Plaintext);
    let server_finished = find_send(&server_flight, Epoch::Handshake);

    client.read_hs(&server_hello).unwrap();
    let mut empty = Vec::new();
    assert!(matches!(
        client.write_hs(&mut empty),
        Some(rustls::quic::KeyChange::Handshake { .. })
    ));
    assert!(empty.is_empty());
    client.read_hs(&server_finished).unwrap();
    ...
    let done = server.read(Epoch::Handshake, &client_finished).unwrap();
...
#[test]
fn shin_quic_client_handshakes_with_rustls_server() {
    drive_shin_quic_client_vs_rustls_server(CipherSuite::Aes128GcmSha256);
}

#[test]
fn shin_quic_server_handshakes_with_rustls_client() {
    drive_shin_quic_server_vs_rustls_client(CipherSuite::Aes128GcmSha256);
}

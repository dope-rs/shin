...
use crate::common::{Server, ServerConfig, find_send, has_done, random_signing_key};

const SERVER_TP: &[u8] = b"server-transport-params";
const CLIENT_TP: &[u8] = b"client-transport-params";

struct Pair {
    client: Client<fn() -> u64>,
    server: Server<fn() -> u64>,
}

fn make_pair() -> Pair {
    let server_key = random_signing_key();
    let server_pubkey = *server_key.pubkey().unwrap();
    let server: Server<fn() -> u64> = Server::new_with_transport(
        ServerConfig {
    ...
    let client: Client<fn() -> u64> = Client::new_with_transport(
        Config {
